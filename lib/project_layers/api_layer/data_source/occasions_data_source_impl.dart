...
class OccasionsDataSourceImpl implements OccasionsDataSource {
  ...
  Future<ApiResult<List<OccasionEntity>>> getOccasions() async {
    try {
      OccasionsResponse occasionsResponse = await apiClient.getOccasions();
      return ApiSuccessResult(
        occasionsResponse.occasions!.map((e) => e.toCategoryEntity()).toList(),
      );
    } catch (e) {
      if (e is DioException) {
        ...
        if (responseData != null && responseData is Map<String, dynamic>) {
          ...
          return ApiErrorResult<List<OccasionEntity>>(message.toString());
        }
        return ApiErrorResult<List<OccasionEntity>>(e.message ?? e.toString());
      }
      return ApiErrorResult<List<OccasionEntity>>(e.toString());
    }
  }
}
