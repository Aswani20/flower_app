  ...
  group("LoginRepositoryImpl", () {
    test(
      ...
      () async {
        // arrange
        when(
          mockDataSource.login(request: anyNamed('request')),
        ).thenAnswer((_) async => tResponse);

        // act
        final result = await repository.login(request: tRequest);

        // assert
        expect(result.isRight(), true);
        expect(result.getOrElse(() => tResponse).token, tResponse.token);
        verify(mockDataSource.login(request: anyNamed('request'))).called(1);
      },
    );

    test("should return Left(ServerError) when credentials invalid", () async {
      // arrange
      when(
        mockDataSource.login(request: anyNamed('request')),
    ...
    test("should return Left(NetworkError) when network fails", () async {
      // arrange
      when(
        mockDataSource.login(request: anyNamed('request')),
    ...
    test("should return Left(ServerError) when server fails", () async {
      // arrange
      when(
        mockDataSource.login(request: anyNamed('request')),
    ...
    test("should return Left(UserError) for unexpected exceptions", () async {
      // arrange
      when(
        mockDataSource.login(request: anyNamed('request')),
